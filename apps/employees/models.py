...
class EmployeeTransfer(OrganizationEntity):
    ...
    def clean(self):
        ...
        if self.department_id and self.department.organization_id != self.organization_id:
            raise ValidationError({'department': "Department must belong to same organization"})
        if self.designation_id and self.designation.organization_id != self.organization_id:
            raise ValidationError({'designation': "Designation must belong to same organization"})
        if self.location_id and self.location.organization_id != self.organization_id:
            raise ValidationError({'location': "Location must belong to same organization"})
        
        if self.to_department_id and self.to_department.organization_id != self.organization_id:
            raise ValidationError({'to_department': "Target Department must belong to same organization"})
        if self.to_location_id and self.to_location.organization_id != self.organization_id:
            raise ValidationError({'to_location': "Target Location must belong to same organization"})
        if self.to_manager_id and self.to_manager.organization_id != self.organization_id:
            raise ValidationError({'to_manager': "Target Manager must belong to same organization"})

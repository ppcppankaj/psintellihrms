...
class EmployeeViewSet(BulkImportExportMixin, OrganizationViewSetMixin, PermissionRequiredMixin, viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not tenant:
                return Response({'detail': 'Organization context missing'}, status=400)
            ...
            if max_employees is not None:
                ...
                if current_count >= max_employees:
                    return Response({
                        'success': False,
                        'message': f'Tenant employee limit reached ({max_employees}). Please upgrade your plan.'
...
class EmployeeDocumentViewSet(OrganizationViewSetMixin, FilterByPermissionMixin, viewsets.ModelViewSet):
    """
    ...
    def perform_create(self, serializer):
        ...
        employee_id = self.request.data.get('employee') or self.request.data.get('employee_id')
        if employee_id:
            serializer.save(employee_id=employee_id, **extra_kwargs)
        else:
            employee = getattr(self.request.user, 'employee', None)
            if employee:
                serializer.save(employee=employee, **extra_kwargs)
            else:
...
class EmploymentHistoryViewSet(OrganizationViewSetMixin, FilterByPermissionMixin, viewsets.ModelViewSet):
    """
...
class CertificationViewSet(OrganizationViewSetMixin, FilterByPermissionMixin, viewsets.ModelViewSet):
    """

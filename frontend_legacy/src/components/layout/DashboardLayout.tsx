...
export default function DashboardLayout() {
    ...
    const tenant = useAuthStore((state) => state.organization)
    const logout = useAuthStore((state) => state.logout)
    const hasPermission = useAuthStore((state) => state.hasPermission)

    // const { totalUnreadCount } = useChat() // Commented out for debugging
    ...
    // const { profile } = useUserProfile(); // Commented out to potentially fix unhandled error

    // DEBUG: Force combine all navigation and add a test item
    const filteredNavigation = [
        ...
        ...superuserNavigation,
        {
            title: 'DEBUG',
            items: [
                { name: 'Debug Item', href: '/dashboard', icon: HomeIcon, permission: null }
            ]
        }
    ];

    console.log('DashboardLayout Rendering Navigation:', filteredNavigation);
    ...
    return (
        <div className="min-h-screen bg-surface-50 dark:bg-surface-900">
            ...
            <aside
                className={clsx(
                    'fixed inset-y-0 left-0 z-50 w-64 transform bg-white dark:bg-surface-800 border-r-4 border-red-500 transition-transform duration-300 ease-in-out lg:translate-x-0',
            ...
            >
                ...
                <nav className="flex-1 px-4 py-6 space-y-6 overflow-y-auto max-h-[calc(100vh-140px)] custom-scrollbar">
                    {filteredNavigation.length === 0 && <div className="text-red-500 font-bold p-4">Navigation is empty!</div>}

                    {filteredNavigation.map((group) => (
                        <div key={group.title} className="space-y-2">
                            <h3 className="px-4 text-xs font-semibold text-surface-400 uppercase tracking-wider bg-yellow-100 dark:bg-yellow-900/30">
                                {group.title}
                            </h3>
                ...
                {/* Quick punch button - hidden for superusers only if explicit */}
                {!(user?.is_superuser) && (
